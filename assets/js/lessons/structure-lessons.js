...
export const structureLessons = {
  ...
  "parameters-arguments": {
    ...
    challenge: { title: "Improve a signature", prompt: "Redesign <code>send(message, true, 3, false)</code> so a reader can tell what each setting means.", solution: "One possible JavaScript call is <code>send(message, { urgent: true, retries: 3, silent: false })</code>. In C# or Python, named arguments may also solve the clarity problem." },
  ...
  objects: {
    ...
    challenge: { title: "Protect an invariant", prompt: "Design a <code>takeHealing</code> method for a player whose <code>health</code> must stay from <code>0</code> through <code>maxHealth</code>.", solution: "Set <code>health</code> to the smaller of <code>maxHealth</code> and <code>health + validated_amount</code>. Decide and document what negative healing means rather than accidentally turning it into damage." },
