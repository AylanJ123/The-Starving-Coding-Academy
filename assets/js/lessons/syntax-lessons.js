...
export const syntaxLessons = {
  ...
  comments: {
    ...
    challenge: { title: "Rewrite the commentary", prompt: "Improve the comment <code>// checks if the player can enter</code> above a rule requiring level <code>12</code>.", solution: "One useful version is <code>// The tournament unlocks at level 12 so new players finish the movement tutorial first.</code> It explains why the threshold exists." },
  ...
  "complex-types": {
    ...
    challenge: { title: "Choose the shape", prompt: "Choose structures for a player's ordered hotbar, the unique IDs of defeated bosses, and a lookup from item ID to item details.", solution: "A sequence for the hotbar, a set for defeated boss IDs, and a map or dictionary for item lookup are natural choices." },
  ...
  booleans: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: ["<strong>NOT</strong> flips a boolean. <strong>AND</strong> requires both sides. <strong>OR</strong> requires at least one. Many languages use short-circuit evaluation and may skip the right side when the left already determines the result."],
      },
    ],
